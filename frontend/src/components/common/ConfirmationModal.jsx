...
export default function ConfirmationModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs" onClick={onClose} aria-hidden="true" />

      <div className="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl border border-slate-200 z-10">
        <div className="flex items-start gap-4">
          ...
          <button onClick={onClose} aria-label="Close modal" className="rounded-lg p-1 text-slate-400 hover:bg-slate-100 cursor-pointer">
            <X className="size-4" />
          </button>
        </div>
        ...
        <div className="mt-6 flex items-center justify-end gap-2.5">
          <button
            ...
            onClick={onClose}
            disabled={isLoading}
            className="rounded-lg border border-slate-200 px-4 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-100 disabled:opacity-50 cursor-pointer"
          >
            {cancelText}
          </button>
          <button
            ...
            onClick={onConfirm}
            disabled={isLoading}
            className={`rounded-lg px-4 py-2 text-xs font-semibold shadow-xs disabled:opacity-50 cursor-pointer ${btnColor}`}
          >
